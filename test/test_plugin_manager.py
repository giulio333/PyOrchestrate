...
class TestPluginManager(unittest.TestCase):
    ...
    def test_register_and_unregister_zmq_plugin(self):
        ...
        self.assertIsNotNone(self.plugin_manager.com)

        with patch.object(zmq_plugin, "finalize", wraps=zmq_plugin.finalize) as mock_finalize:
            self.plugin_manager.unregister()
            mock_finalize.assert_called_once()

        self.assertIsNone(self.plugin_manager.com)
    ...
    def test_zmq_context_closed_on_unregister(self):
        ...
        with patch.object(zmq_plugin.context, "term", wraps=zmq_plugin.context.term) as mock_term:
            self.plugin_manager.unregister()
