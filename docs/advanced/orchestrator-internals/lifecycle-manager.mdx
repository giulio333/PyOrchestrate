...
```text
REGISTERED -> STARTING -> RUNNING -> STOPPING -> TERMINATED
                 |          |
                 +-> FAILED +-> QUARANTINED
```
...
start_agent
    |
    +--> prepare_start() -> immutable generation attempt
    |
    +--> execute_start(attempt)
            |
            +--> AgentEntry._initialize_instance(attempt)
            |
            +--> AgentEntry._start_instance(attempt)
            |
            +--> wait for entry.state_events.start_event
```
...
`Orchestrator.join()` for the normal orchestrator lifecycle. Startup-timeout
