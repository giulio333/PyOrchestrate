...
Execution mode is selected by the registered agent class, not by an orchestrator configuration flag:
...
# Subclass PeriodicProcessAgent for a process-backed agent.
class CpuWorker(PeriodicProcessAgent):
    ...

# Subclass PeriodicThreadAgent for a thread-backed agent.
class IoWorker(PeriodicThreadAgent):
    ...
```
...
- [DependencyGraph](/advanced/orchestrator-internals/dependency-graph) - Determines agent startup order
- [WorkerPoolScheduler](/advanced/orchestrator-internals/worker-pool) - Manages concurrent agent execution
- [PluginManager](/advanced/utilities/plugin-manager) - Plugin lifecycle management
- [MessageChannel](/advanced/utilities/message-channel) - Inter-agent communication
- [Orchestrator Internals](/advanced/orchestrator-internals/index) - Architecture overview
