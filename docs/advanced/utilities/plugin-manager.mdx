...
class MyOrchestrator(Orchestrator):
    class Plugin(Orchestrator.Plugin):
        # Declare orchestrator-level plugins here.
        pass

# The inherited constructor creates the PluginManager, assigns its owner,
# and initializes declared plugins.
orchestrator = MyOrchestrator(
    config=MyOrchestrator.Config(enable_command_interface=False)
)
```
