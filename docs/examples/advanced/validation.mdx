...
def run_validation_example():
    ...
    # Start all agents. The one that fails validation is started too: it
    # fails inside BaseAgent.run(), once its process is already up, and
    # terminates with AgentTerminationStatus.ERROR.
    orchestrator.start()
