...
For a detailed explanation of how validation works in PyOrchestrate, please refer to the [Validation](/learn/config_and_validation#validation) page.
</Tip>
...
def run_validation_example():
    ...
    # Start all three agents: validation runs inside each agent, after start
    orchestrator.start()
...
All three agents are started. Registration does not validate anything: an agent
validates inside `BaseAgent.run()`, after its process has already been started
and before plugin initialization, `setup()` and `execute()`. So
`simple_agent_error` starts like the others, fails validation on
`threshold=150`, and terminates with `AgentTerminationStatus.ERROR` without ever
reaching `execute()`. It also reports the failure to the orchestrator, so the
message reaches `pyorchestrate history --type agent_error` and the callbacks
registered for `OrchestratorEvent.AGENT_ERROR`.

`simple_agent_warning` passes with a warning and runs normally. See
[Configuration and Validation](/learn/config_and_validation) for the full
policy.
