...
it before running this project:
...
<Tip>
**Use the capitalized `Config` attribute**

When adding a new agent, override the inner `Config` attribute — capital `C` —
to use a custom configuration class, as shown with `WCConfig`. Do not assign the
lowercase `config` attribute instead. `BaseAgent.__init__` replaces it with the
provided configuration or an instance of `self.Config()`. Otherwise, the agent
silently uses its base configuration and `self.config.url` does not exist.
</Tip>
