...
In the `configurations` directory, create `weather_config.py` with a configuration class named `WCConfig`:
...
Next, in the `models` directory, create `weathercollector.py` with an agent called `WeatherCollector`:
