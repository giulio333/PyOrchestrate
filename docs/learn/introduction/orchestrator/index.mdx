...
Routed lifecycle events reach both the registered callbacks and the event store, so the history covers the agent lifecycle. The store is bounded: with more than `history_max_events` records, the oldest are dropped.
