...
<img src="/learn/config_l.svg" alt="alt text" className="block dark:hidden" />
<img src="/learn/config_d.svg" alt="alt text" className="hidden dark:block" />
...
The `config: Config` annotation below the inner class is what a type checker and
an editor read to give you *your* attributes on `self.config` instead of the base
class's. PyOrchestrate ships the [PEP 561](https://peps.python.org/pep-0561/)
marker, so this holds for an installed copy of the package too, not only inside
the repository.
...
<Tip>
All **built-in attribute** are well documented. For example, see [LoopingAgent Configuration](/learn/agents/built-in-agents/loopingagent#configuration):
</Tip>
...
The Orchestrator validates its configuration during `Orchestrator(...)` construction. An Agent validates inside `BaseAgent.run()`, after its thread/process has been started but before plugin initialization, `setup()` and `execute()`.

If something looks wrong, validation can **stop the component's normal execution** or just **warn** you, depending on the validation policy.

An agent stopped by a blocking error does not fail silently: before terminating with an `ERROR` status it reports the failure to the orchestrator like any other agent error. The message reaches the event history (`pyorchestrate history --type agent_error`) and triggers the callbacks registered for `OrchestratorEvent.AGENT_ERROR`, carrying the validation message as `error_message`. See [Event Bus](/advanced/orchestrator-internals/event-bus).
...
| `ERROR`        | Normal execution is aborted unless the policy ignores errors. |
...
You know that the component reaches setup and execution only after its configuration has passed the active policy.
