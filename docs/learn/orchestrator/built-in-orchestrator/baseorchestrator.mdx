...
| `agent_start_timeout` | `30.0` | Maximum time to wait for agent startup |
...
`OrchestratorEventBus` combines event dispatch with bounded event history.

### Message Router

`MessageRouter` consumes agent messages and forwards lifecycle information to the event system.
