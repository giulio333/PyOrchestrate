...
| `agent_start_timeout` | `30.0` | Timeout for `start_event` only when registration supplied explicit state events |
...
`OrchestratorEventBus` combines event dispatch with bounded event history. Agent messages currently reach the underlying `EventManager` directly, so their lifecycle callbacks are not automatically recorded.

### Message Router

`MessageRouter` consumes agent messages and forwards lifecycle information to the callback manager.
