...
`agent_class` must be an agent in its process or thread flavour: the orchestrator starts it, polls it for liveness and joins it, and those three come from the flavour rather than from `BaseAgent`. A class without one is refused with `TypeError` here, at registration, instead of failing once the orchestrator is already running.
