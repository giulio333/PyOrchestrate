...
To learn more about `LoopingAgent` click [here](/learn/agents/built-in-agents/loopingagent).
...
sequenceDiagram
    ...
    Agent->>MessageChannel: send(AGENT_READY)
    MessageChannel->>Orchestrator: forward message
...
This method is called by the `run()` method and drives the loop, calling [cycle](#cycle) on each iteration until the configured `limit` is reached.
...
<Warning title="It does not run inside the agent">
`on_stop()` is called by whoever calls `stop()`, which on the orchestrator path is the orchestrator itself — a different **thread** for a thread agent, and a different **process** for a process agent. In the process case it runs against a copy: closing a handle or setting an attribute on `self` there has no effect on the agent that is actually running.

Put cleanup that must happen inside the agent in [on_close](#on-close), which runs in the agent's own context at the end of its lifecycle, or have the agent react to `control_events.stop_event` itself.
</Warning>
