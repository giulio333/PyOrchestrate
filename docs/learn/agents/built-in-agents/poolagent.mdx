...
The default `command_zmq_address` is `tcp://*:5555` for every orchestrator, and the top-level one has already bound it. A pool that enabled the command interface on that address would die on startup with `Address already in use`. That is why the inner orchestrator runs without a command interface unless you provide an `orchestrator_config` of your own — and if you do, give it an address nobody else uses:
...
The supervision step. It calls `pre_runner`, checks whether every child has stopped — and if so stops the pool — otherwise calls `post_runner`.
