...
Creates the inner `Orchestrator`, registers every entry in `config.agents_entry` and starts it. Override it to add your own initialisation, always calling `super().setup()` first.
