...
Every user-defined agent inherits from one of the [built-in agents](/learn/agents/built-in-agents/baseagent), in its **process** or **thread** flavour: `BaseProcessAgent` or `BaseThreadAgent`, `LoopingProcessAgent` or `LoopingThreadAgent`, and so on.

<Warning>
Do not inherit from `BaseAgent` itself. It defines the lifecycle hooks and nothing to run them with — the flavour is what brings `start()`, `join()` and `is_alive()` — so registering a direct subclass of it raises `TypeError`.
</Warning>
...
<Tip>
`PoolAgent` is the one branch that changes the shape of the tree: instead of running your logic, it starts an orchestrator of its own and supervises a group of child agents. See [PoolAgent](/learn/agents/built-in-agents/poolagent).
</Tip>
