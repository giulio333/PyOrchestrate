...
- **System inspection**: `status` - Get an orchestrator report (the agent-specific path has a current handler defect)
- **Dependencies**: `dependencies` - Intended to show the dependency graph; currently affected by the same handler defect
...
<Warning title="Current inspection limitations">
`status AGENT_NAME` and `dependencies` still reference a removed source attribute and can return an internal error. Web endpoints backed by those two commands have the same limitation. Also, routed lifecycle callbacks are not automatically present in event history. See [Runtime Commands](/cli/runtime-commands) for details.
</Warning>
...
- **Network binding**: Use `tcp://127.0.0.1:5555` when the command interface should only be reachable locally
- **Remote exposure**: Binding to `tcp://*:5555` exposes the command interface on every network interface
